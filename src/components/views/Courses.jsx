...
export default function Courses () {
  ...
  async function handleSave () {
    ...
    const payload = {
      ...
      name: form.name,
      instructor: form.instructor,
      grade: letterFromPercent(percent),
      credits: Number(form.credits),
      semester: form.semester,
      color: form.color,
      status: form.status
    ...
    <div className='p-6'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6'>
        {[
          ...
          {
            label: 'Credit hours',
            value: courseList.reduce((s, c) => s + (c.credits || 0), 0)
          },
          {
            label: 'Current GPA',
          ...
          {
            label: 'Completed',
            value: courseList.filter(c => c.status === 'completed').length
          }
        ].map(({ label, value }) => (
          <Card
            key={label}
      ...
      <div className='flex gap-2 mt-6'>
        {[
          ...
          {
            key: 'active',
            count: courseList.filter(c => (c.status ?? 'active') === 'active')
              .length
          },
          {
            key: 'completed',
            count: courseList.filter(c => c.status === 'completed').length
          }
        ].map(({ key, count }) => (
          <button
            key={key}
            onClick={() => setFilter(key)}
            className={`text-xs px-3 py-1 rounded-md border cursor-pointer transition-colors capitalize ${
              filter === key
          ...
          >
            ...
            <div className='border-t pt-2 border-cream3 flex flex-row justify-between items-center'>
              <div className='flex items-center gap-1.5'>
                ...
                <span
                  className={`text-[10px] font-medium rounded-md py-0.5 px-2 ${
                    (el.status ?? 'active') === 'completed'
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='flex gap-3'>
            <div className='flex flex-col gap-2 flex-1'>
              ...
              <div className='flex gap-2 items-center h-8.5'>
              ...
              <div className='flex gap-2'>
                {['active', 'completed'].map(s => (
                  <button
                    ...
                    className={`text-[11px] font-medium px-3 py-1 rounded-md border cursor-pointer transition-colors ${
                      form.status === s
                        ? s === 'completed'
                          ? 'bg-sage-light text-sage-dark border-sage'
                          : 'bg-sky-light text-sky border-sky'
                        : 'bg-transparent text-text-muted border-border-strong hover:bg-cream2'
                    }`}
                  >
